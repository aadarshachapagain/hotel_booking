...
class VirtualPoint(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        count=VirtualPoint.objects.exclude(id=self.id).filter(user=self.user.user_id).count()
        if count > 0:
            Virtual_instance = VirtualPoint.objects.exclude(id=self.id).filter(user=self.user.user_id).latest('created_at')
            self.net_total = Virtual_instance.net_total + self.transaction_amount
            
        else:
            self.net_total = self.transaction_amount
        return super(VirtualPoint, self).save()
